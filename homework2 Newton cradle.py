...
for i in range(5):
    ...
    #單擺的輕繩
    springs.append(cylinder(radius=0.01))
...
while True:
    ...
    for ball in balls:
        Ek += 0.5 * m * ball.v.x ** 2
    Total_Kinetic_Energy.plot(pos=(t, Ek))
    #平均動能隨時間關係圖
    Ek_total += Ek*dt
    Average_Kinetic_Energy.plot(pos=(t, Ek_total/t))
    #動能變數歸零
    Ek = 0
    #接著畫出位能隨時間的關係圖
    for ball in balls:
        Eg += m*g*(ball.pos.y+L+m*g/k)
    Total_Gravitation_Energy.plot(pos=(t, Eg))
    #還有平均位能隨時間關係圖
    Eg_total += Eg*dt
    Average_Gravitation_Energy.plot(pos=(t, Eg_total/t))
    #位能變數歸零
    Eg = 0
    for i in range(5):
